---
name: P12-conflict-across-three-requirements
tags: [positive, n-ary, constitution, gateway]
...
We are missing the report latency budget whenever the policy service is slow.
When the policy service is slow, serve the report from cache so the budget is
never missed.
