...
Conflict under test: 'the first declined authorisation' is 'a payment fails' and 'abandon dunning' is 'do not retry'. R4 requires a retry every 24 hours for 3 days on that trigger; the intent requires no retry at all. Both the trigger and the disposition are renamed, so nothing matches lexically.
