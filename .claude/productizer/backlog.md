...
| B12 | A committed learnings store, subordinate to the spec | `todo` | — | maintainer, 2026-08-29 | Not everything learned is a requirement. "The build breaks unless you run X first" is nobody's obligation and forcing it into `spec.md` corrupts the one thing that file is for - so there is a storage gap, not only a retrieval one. Put it in the repo (`.claude/productizer/learnings.jsonl`), committed and append-only, one entry per line with provenance, a status and optional requirement ids. **It sits below the spec and cannot outrank it**: a learning informs, it never obligates. A learning that contradicts an active requirement is a finding, not a fact - it goes through intake like any other intent, the same route Stage 9 already uses for code-versus-spec drift. One that turns out to be a real obligation graduates to an `R` id and stops being a learning. Two rules from an earlier implementation of ours, both about not trusting yourself: **unverified until independently corroborated** ("the same dispatch can never confirm its own learning, or an agent would simply graduate whatever it just wrote"), and **dormancy stops serving, never deletes** - with a missing timestamp never read as infinitely old, since that hides the entries with the least provenance to recover them by. Its threshold is derived from a corpus audit, not chosen, and it records the rejected alternative: graduating anything carrying a commit sha, refused because every dispatch that lands code has one and "a gate that passes everything is the same as no gate". Productizer can do one thing those cannot - a learning may cite `R14`, and because ids are permanent, "this learning is about a requirement that has since been superseded" becomes mechanically detectable, the same check `drift-reverse.sh` already runs over code. That was the wrong model: its memory is a KV table served alphabetically with no timestamps |
